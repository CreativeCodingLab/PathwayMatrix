...
import static main.MainPathwayViewer_1_7.minerList;
import static main.MainPathwayViewer_1_7.minerNames;
...
public class Venn_Overview{
	...
	public void initialize() {
		...
		for (int i=0;i<minerList.size();i++){
			if (main.MainPathwayViewer_1_7.pairs[i].size()>0)
				numMinerContainData++;
		}	
		...
		for (int i=0;i<minerList.size();i++){
			if (main.MainPathwayViewer_1_7.pairs[i].size()>0){
				...
				pair2[count] = main.MainPathwayViewer_1_7.pairs[i];
	...
	*/
	...
	public void draw(float xPanelRight, float yy4, int numSongs) {
        ...
        for (int i = 0; i < centers.length; i++) {
            ...
            Color color = new Color(main.MainPathwayViewer_1_7.colorRelations[minerGlobalIDof[i]]);  
            
             // if (deactive[i])
           // 	color = new Color(255,255,255,10);
            
            if (minerGlobalIDof[i]==main.MainPathwayViewer_1_7.currentRelation){
            	currentSize = radius;
            }
        ...
        if (minerGlobalIDof!=null && brushing>=0)
        	main.MainPathwayViewer_1_7.currentRelation = minerGlobalIDof[brushing];
        else
        	main.MainPathwayViewer_1_7.currentRelation = -1;
	...
	public boolean mouseClicked() {
		if (brushing>=0){
			...
			main.MainPathwayViewer_1_7.currentRelation = -2;
			return false;
		}	
	}

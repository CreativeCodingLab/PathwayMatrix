...
import static main.MainViewer.minerNames;
...
public class Venn_Overview{
	...
	public void compute() {
		...
		for (int i=0;i<minerList.size();i++){
			if (main.MainViewer.pairs[i].size()>0)
				numMinerContainData++;
		}	
		...
		for (int i=0;i<minerList.size();i++){
			if (main.MainViewer.pairs[i].size()>0){
				...
				pair2[count] = main.MainViewer.pairs[i];
	...
	*/
	...
	public void draw(float xPanelRight, float yy4, int numSongs) {
        ...
        for (int i = 0; i < centers.length; i++) {
           ...
           // 	color = new Color(255,255,255,10);
            ...
            if (minerGlobalIDof[i]==main.MainViewer.currentRelation){
            	currentSize = radius;
            }
	...
	public boolean mouseClicked() {
		if (brushing>=0){
			main.MainViewer.currentRelation = minerGlobalIDof[brushing];
			deactive[brushing] =! deactive[brushing];
			return true;
		}
		else{
			main.MainViewer.currentRelation = -2;
			return false;
		}	
	}

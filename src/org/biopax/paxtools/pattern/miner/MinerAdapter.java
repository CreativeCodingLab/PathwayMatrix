...
public abstract class MinerAdapter implements Miner
{
	...
	public void writeResultAsSIF(Map<BioPAXElement, List<Match>> matches, OutputStream out,
		...
		System.out.println("processingMiner="+main.MainViewer.processingMiner+"	"+this.getName()+"	directed="+directed);
	...
	public void writeSIFsUsingSIFFramework(Map<BioPAXElement, List<Match>> matches,
		OutputStream out, boolean directed) throws IOException{
	...
	public static void storeData(String rel, String gene1, String gene2){
		...
		if (!main.MainViewer.pairs[main.MainViewer.processingMiner].contains(rel)){
			main.MainViewer.pairs[main.MainViewer.processingMiner].add(rel);
		}	
		//String minerName = main.MainViewer.minerList.get(main.MainViewer.processingMiner).toString();
		
		if (!main.MainViewer.genes[main.MainViewer.processingMiner].contains(gene1)){
			main.MainViewer.genes[main.MainViewer.processingMiner].add(gene1);
		}
		if (!main.MainViewer.genes[main.MainViewer.processingMiner].contains(gene2)){
			main.MainViewer.genes[main.MainViewer.processingMiner].add(gene2);
		}
		
		// Global data 
		if (!main.MainViewer.allGenes.contains(gene1)){
			main.MainViewer.allGenes.add(gene1);
			main.MainViewer.ggg.add(new Gene(gene1,main.MainViewer.ggg.size()));
		}
		if (!main.MainViewer.allGenes.contains(gene2)){
			main.MainViewer.allGenes.add(gene2);
			main.MainViewer.ggg.add(new Gene(gene2, main.MainViewer.ggg.size()));
		}
	...
	{
		System.out.println("writeResultDetailed="+main.MainViewer.processingMiner+

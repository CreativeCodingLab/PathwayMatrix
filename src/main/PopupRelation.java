...
public class PopupRelation{
	...
	public PopupRelation(PApplet parent_){
		parent = parent_;
		items = new String [PathwayViewer_2_8.minerList.size()];
		for (int i = 0; i < PathwayViewer_2_8.minerList.size(); i++) {
			items[i] = PathwayViewer_2_8.minerList.get(i).toString();
		}	
		s = new boolean[PathwayViewer_2_8.minerList.size()];
	}
		...
		if (b>=0){
			...
			for (int i=0;i<items.length;i++){
				if (s[i]){
					...
					parent.fill(main.PathwayViewer_2_8.colorRelations[i]);
				}
				else if (i==b){
					parent.fill(main.PathwayViewer_2_8.colorRelations[i]);
				}
				...
				float r = PApplet.map(PApplet.sqrt(main.PathwayViewer_2_8.pairs[i].size()), 0, max, 0, 25);

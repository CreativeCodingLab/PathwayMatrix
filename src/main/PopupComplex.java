...
public class PopupComplex{
	...
	public void setItems(){
		...
		s=-400;
		for (int i=0; i<main.PathwayViewer_2_7.complexList.size();i++){
			Complex com = main.PathwayViewer_2_7.complexList.get(i);
			int size = main.PathwayViewer_2_7.proteinsInComplex[i].size();
	...
	public void draw(float x_){
	 ...
	 public ArrayList<Integer> getUpSreamSetId(Map.Entry<Complex, Integer> entry, int indexSet, int indexHash) {
		 ...
		 for (Map.Entry<Complex, Integer> entryParent : itemHash.entrySet()) {
			 if (indexHashParent<indexHash){
				 ...
				 ArrayList<String> components = main.PathwayViewer_2_7.getComplexById(indexSetParent);
	 ...
	 public void drawRelationshipUpStream(Map.Entry<Complex, Integer> entry, int indexSet, int indexHash, int r, int g, int b, int alpha, boolean recursive, int level) {
		 ...
		 for (int i=0;i<parentSetIDs.size();i++){
			...
			float num = main.PathwayViewer_2_7.proteinsInComplex[indexSet].size();
	...
	public void drawRelationshipDownStream(int indexSet, int indexHash, int r, int g, int b, int alpha, boolean recursive, int level) {
		 ArrayList<String> components = main.PathwayViewer_2_7.getComplexById(indexSet);
		 for (int i=0;i<components.size();i++){
			 if (main.PathwayViewer_2_7.mapComplexRDFId_index.get(components.get(i))!=null){
				 int indexSet2 = main.PathwayViewer_2_7.mapComplexRDFId_index.get(components.get(i));
				...
				float num = main.PathwayViewer_2_7.proteinsInComplex[indexSet2].size();
				float thickness = PApplet.map(PApplet.sqrt(num), 0, PApplet.sqrt(maxSize), 0, maxH/2);
				int g2 = g+0*level;
				if (g2>255)
					g2=255;
	...
	public int getIndexInSet(int brushing) {
		...
		for (i=0; i<main.PathwayViewer_2_7.complexList.size();i++){
			Complex com = main.PathwayViewer_2_7.complexList.get(i);
			if (com.getDisplayName().equals(name))
				return i;
		}
	...
	public int getIndexInHash(int indexSet) {
		String name = main.PathwayViewer_2_7.complexList.get(indexSet).getDisplayName();

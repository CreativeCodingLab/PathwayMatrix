...
public class PopupComplex{
	...
	public void setItems(){
		...
		s=-400;
		for (int i=0; i<main.PathwayMatrix_1_1.complexList.size();i++){
			Complex com = main.PathwayMatrix_1_1.complexList.get(i);
			int size = main.PathwayMatrix_1_1.proteinsInComplex[i].size();
	 ...
	 public ArrayList<Integer> getUpSreamSetId(Map.Entry<Complex, Integer> entry, int indexSet, int indexHash) {
		 ...
		 for (Map.Entry<Complex, Integer> entryParent : itemHash.entrySet()) {
			 if (indexHashParent<indexHash){
				 ...
				 ArrayList<String> components = main.PathwayMatrix_1_1.getComplexById(indexSetParent);
	...
	public void drawRelationshipDownStream(int indexSet, int indexHash, int r, int g, int b, int alpha, boolean recursive, int level) {
		 ArrayList<String> components = main.PathwayMatrix_1_1.getComplexById(indexSet);
		 for (int i=0;i<components.size();i++){
			 if (main.PathwayMatrix_1_1.mapComplexRDFId_index.get(components.get(i))!=null){
				 int indexSet2 = main.PathwayMatrix_1_1.mapComplexRDFId_index.get(components.get(i));
	...
	public int getIndexInSet(int brushing) {
		...
		for (i=0; i<main.PathwayMatrix_1_1.complexList.size();i++){
			Complex com = main.PathwayMatrix_1_1.complexList.get(i);
			if (com.getDisplayName().equals(name))
				return i;
		}
	...
	public int getIndexInHash(int indexSet) {
		String name = main.PathwayMatrix_1_1.complexList.get(indexSet).getDisplayName();

...
public class WordCloud{
	...
	public void mouseClicked(){
		if (b>=0){
			if (s==b){
				...
				main.PathwayViewer_2_7.popupReaction.mouseMoved();  // to update protein names in current reactions
			}
			else{
				s=b;
	...
	class Tag {
		...
		Tag(Tag[] others, int id, String str, int count, int maxCount,  int x, int y) {
			...
			float v = PApplet.map(count, 0, maxCount, 0.5f, 1);
			
			color = new Color(0,0,0, v);

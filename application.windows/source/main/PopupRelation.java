...
public class PopupRelation{
	...
	public PopupRelation(PApplet parent_){
		parent = parent_;
		items = new String [MainMatrixVersion_1_3.minerList.size()];
		for (int i = 0; i < MainMatrixVersion_1_3.minerList.size(); i++) {
			items[i] = MainMatrixVersion_1_3.minerList.get(i).toString();
		}	
		s = new boolean[MainMatrixVersion_1_3.minerList.size()];
	}
	...
	public void draw(float x_){
		...
		if (b>=0){
			...
			float max =-1;
			for (int j=0;j<main.MainMatrixVersion_1_3.pairs.length;j++){
				float sqrt = PApplet.sqrt(main.MainMatrixVersion_1_3.pairs[j].size());
				if (sqrt>max)
					max = sqrt;
			}
			for (int i=0;i<items.length;i++){
				if (s[i]){
					...
					parent.fill(main.MainMatrixVersion_1_3.colorRelations[i]);
				}
				else if (i==b){
					parent.fill(main.MainMatrixVersion_1_3.colorRelations[i]);
				}
				...
				float r = PApplet.map(PApplet.sqrt(main.MainMatrixVersion_1_3.pairs[i].size()), 0, max, 0, 25);

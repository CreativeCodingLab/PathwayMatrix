...
public class PopupRelation{
	...
	public PopupRelation(PApplet parent_){
		parent = parent_;
		items = new String [PathwayMatrix_1_1.minerList.size()];
		for (int i = 0; i < PathwayMatrix_1_1.minerList.size(); i++) {
			items[i] = PathwayMatrix_1_1.minerList.get(i).toString();
		}	
		s = new boolean[PathwayMatrix_1_1.minerList.size()];
	}
		...
		if (b>=0){
			...
			for (int i=0;i<items.length;i++){
				if (s[i]){
					...
					parent.fill(main.PathwayMatrix_1_1.colorRelations[i]);
				}
				else if (i==b){
					parent.fill(main.PathwayMatrix_1_1.colorRelations[i]);
				}
				...
				float r = PApplet.map(PApplet.sqrt(main.PathwayMatrix_1_1.pairs[i].size()), 0, max, 0, 25);

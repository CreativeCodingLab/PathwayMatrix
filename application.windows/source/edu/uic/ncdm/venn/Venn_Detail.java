...
import static main.MainMatrixVersion_1.minerList;
...
public class Venn_Detail{
	...
	public void compute(int current) {
		...
		for (int j=0;j<minerList.size();j++){
			int countPair=0;
			for (int i2 =0; i2< main.MainMatrixVersion_1.pairs[current].size(); i2++){
				for (int j2 =0; j2< main.MainMatrixVersion_1.pairs[j].size(); j2++){
					if (main.MainMatrixVersion_1.pairs[current].get(i2).equals(main.MainMatrixVersion_1.pairs[j].get(j2)))
						countPair++;
				}
			}
			if (countPair>0){
		...
		for (int i =0; i< numMiner; i++){
			...
			areas[i]= main.MainMatrixVersion_1.pairs[minerID.get(i)].size();
		}
		
		int k=numMiner;
		for (int i =0; i< numMiner; i++){
			for (int j =i+1; j< numMiner; j++){
				for (int i2 =0; i2< main.MainMatrixVersion_1.pairs[minerID.get(i)].size(); i2++){
					for (int j2 =0; j2< main.MainMatrixVersion_1.pairs[minerID.get(j)].size(); j2++){
						if (main.MainMatrixVersion_1.pairs[minerID.get(i)].get(i2).equals
								(main.MainMatrixVersion_1.pairs[minerID.get(j)].get(j2))){
							areas[k]++;
						}
					}
				}	
	...
	public void draw(float x4, float y4, int numSongs) {
        ...
        for (int i = 0; i < centers.length; i++) {
            ...
            if (minerID.get(i)==main.MainMatrixVersion_1.currentRelation){
            ...
            Color color = new Color(main.MainMatrixVersion_1.colorRelations[minerGlobalIDof[i]]);  

...
import static main.MainMatrixVersion_1.minerNames;
...
public class Venn_Overview{
	...
	public void initialize() {
		...
		for (int i=0;i<minerList.size();i++){
			if (main.MainMatrixVersion_1.pairs[i].size()>0)
				numMinerContainData++;
		}	
		...
		for (int i=0;i<minerList.size();i++){
			if (main.MainMatrixVersion_1.pairs[i].size()>0){
				...
				pair2[count] = main.MainMatrixVersion_1.pairs[i];
	...
	*/
	...
	public void draw(float xPanelRight, float yy4, int numSongs) {
        ...
        for (int i = 0; i < centers.length; i++) {
           ...
           // 	color = new Color(255,255,255,10);
            ...
            if (minerGlobalIDof[i]==main.MainMatrixVersion_1.currentRelation){
            	currentSize = radius;
            }
        ...
        if (minerGlobalIDof!=null && brushing>=0)
        	main.MainMatrixVersion_1.currentRelation = minerGlobalIDof[brushing];
        else
        	main.MainMatrixVersion_1.currentRelation = -1;
	...
	public boolean mouseClicked() {
		if (brushing>=0){
			...
			main.MainMatrixVersion_1.currentRelation = -2;
			return false;
		}	
	}

...
import static main.MainMatrixVersion_1_2.pairs;
import static main.MainMatrixVersion_1_2.ggg;
import static main.MainMatrixVersion_1_2.geneRelationList;
import static main.MainMatrixVersion_1_2.gene_gene_InComplex;
import static main.MainMatrixVersion_1_2.maxGeneInComplex;
import static main.MainMatrixVersion_1_2.leaderSortedMap;
import static main.MainMatrixVersion_1_2.locals;
...
//	public int orderReading;
	...
	public Gene(String name_, int order_){
		name = name_;
		iX = new Integrator(main.MainMatrixVersion_1_2.marginX,.5f,.1f);
		iY = new Integrator(main.MainMatrixVersion_1_2.marginY,.5f,.1f);
	...
	public static void compute(){
		 ...
		 for (int i=0; i<main.MainMatrixVersion_1_2.ggg.size();i++){
			 hGenes.put(main.MainMatrixVersion_1_2.ggg.get(i).name, new int[numMinerContainData]);
		 }
	...
	public static void computeGeneGeneInComplex(){
		...
		for (int c=0;c<main.MainMatrixVersion_1_2.complexSet.size();c++){
			ArrayList<String> a = main.MainMatrixVersion_1_2.proteinsInComplex[c];
			for (int i=0;i<ggg.size();i++){
				for (int j=0;j<ggg.size();j++){
	...
	public static void orderBySimilarity(){	
		...
		for (int p=0;p<ggg.size();p++){
			if (main.MainMatrixVersion_1_2.isSmallMolecule(ggg.get(p).name)){
		...
		// Eliminate proteins
		for (int i=0;i<ggg.size();i++){
			if (!main.MainMatrixVersion_1_2.isSmallMolecule(ggg.get(i).name)){
	...
	*/
	...
	public static int getSimilarGene(int orderReading1, ArrayList<Integer> a){
		...
		for (int i=0;i<ggg.size();i++){
			...
			if (orderReading1==orderReading2) continue;
			if (a.contains(orderReading2)) continue;
			float dis = computeDis(orderReading1,orderReading2, main.MainMatrixVersion_1_2.popupOrder.slider.val);
			if (dis<minDis){
				minDis = dis;
				minIndex = i;
			}

...
public class ReactionView{
	...
	public void setItems(){
		...
		for (BiochemicalReaction current : main.PathwayViewer_2_9.reactionSet){
			Object[] s = current.getLeft().toArray();
			
			// Compute size of reaction
			int size = 0;
			for (int i3=0;i3<s.length;i3++){
				  String name = main.PathwayViewer_2_9.getProteinName(s[i3].toString());
				  if (name!=null){
					  size++;
				  }	  
				  else if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(s[i3].toString())!=null){
					  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(s[i3].toString());
					  ArrayList<String> components = main.PathwayViewer_2_9.proteinsInComplex[id];
		...
		int numValid = main.PathwayViewer_2_9.mapProteinRDFId.size();
		mapProteinRDFId_index = new HashMap<String,Integer>();
		int p1=0;
		for (Map.Entry<String,String> entry : main.PathwayViewer_2_9.mapProteinRDFId.entrySet()){
		...
		if (popupReactionOrder.s==0 || popupReactionOrder.s==1){
	...
	public void updateComplexList(){
		...
		for (int r=0;r<rectList.size();r++) {
			...
			for (int i3=0;i3<aLeft.length;i3++){
				  if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(aLeft[i3].toString())!=null){
					  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(aLeft[i3].toString());
					  if (id>maxID)
						  maxID =id;
			...
			for (int i3=0;i3<aRight.length;i3++){
				  if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(aRight[i3].toString())!=null){
					  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(aRight[i3].toString());
					  if (id>maxID)
						  maxID =id;
					  if(complexList.indexOf(id)<0)
						  complexList.add(id);
	...
	public ArrayList<String> getUnidentifiedElements2(Object[] s) {
		...
		for (int i=0;i<s.length;i++){
			  String name = main.PathwayViewer_2_9.getProteinName(s[i].toString());
			  if (mapProteinRDFId_index.get(name)!=null){
			  }
			  else  if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(s[i].toString())!=null){
				  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(s[i].toString());
				  ArrayList<String> components = main.PathwayViewer_2_9.proteinsInComplex[id];
	...
	public void updateProteinPositions(){
		...
		if (popupReactionOrder.s==0){
			...
			for (int r=0;r<rectList.size();r++) {
				...
				float size = 0;
				for (int i3=0;i3<aLeft.length;i3++){
					  String name = main.PathwayViewer_2_9.getProteinName(aLeft[i3].toString());
					  if (name==null)
						  name = aLeft[i3].toString();
					  if (mapProteinRDFId_index.get(name)!=null){
						  if (!main.PathwayViewer_2_9.isSmallMolecule(name)) {
							  int p =mapProteinRDFId_index.get(name);
							  score += iP[p].target;
							  size++;
						  }
					  }
					  else  if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(aLeft[i3].toString())!=null){
						  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(aLeft[i3].toString());
				...
				for (int i3=0;i3<aRight.length;i3++){
					  String name = main.PathwayViewer_2_9.getProteinName(aRight[i3].toString());
					  if (name==null)
					  ...
					  else  if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(aRight[i3].toString())!=null){
						  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(aRight[i3].toString());
			...
			hProtein = (parent.height-yBeginList-2)/(numActiveProtein);  // Save 20 pixels for Unidentified elements
			if (hProtein>maxH)
				hProtein =maxH;
	...
	public int getSimilarProtein(ArrayList<Integer> reactProteinList, ArrayList<Integer> a, int[][] scoresComplex, int[][] scoresReaction){
		...
		for (int p=0;p<proteins.length;p++){
			...
			for (int i=0;i<a.size();i++){
				...
				if (!main.PathwayViewer_2_9.isSmallMolecule(proteins[p])) // do not include small molecules into reactions
	...
	public int[][] computeScoreComplex(){
		...
		for (int c=0;c<complexList.size();c++){
			 ArrayList<String> components = main.PathwayViewer_2_9.proteinsInComplex[complexList.get(c)];
			 for (int k=0;k<components.size();k++){
	...
	public void updateComplexPositions(){
		if (popupReactionOrder.s==0 || popupReactionOrder.s==1){
			...
			float[] cPosistions = new float[main.PathwayViewer_2_9.mapComplexRDFId_index.size()];
			int[] cCount = new int[main.PathwayViewer_2_9.mapComplexRDFId_index.size()];
			for (int c=0; c<main.PathwayViewer_2_9.mapComplexRDFId_index.size(); c++){
				cPosistions[c] = 0;
				cCount[c] = 0;
			}
		...
		if (proteins==null) return;
			...
			if (simulationRectList.size()>0){
				...
				for (int i=0;i<interElements.size();i++){
					...
					parent.fill(0);
					String name = main.PathwayViewer_2_9.getProteinName(ref);
					  if (name==null){
					  ...
					  else if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref)!=null){
						  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref);
						  name = main.PathwayViewer_2_9.proteinsInComplex[id].toString();
					  }
...
// ****************** Draw Button simulations ***********************************************************************************************
	...
	public void computeShortestPath(){
		...
		if (bRect>=0 || bProteinL>=0 || bComplexL>=0){
			...
			for (int i=0;i<proteinsDownStreamList.size();i++){
				...
				if (!main.PathwayViewer_2_9.isSmallMolecule(proteins[p])){
					parent.textSize(12);
	...
	public void listProteinDownStream(int r, int recursive, ArrayList<Integer> downstreamList,ArrayList<Integer> levelDownStreamList, ArrayList<Integer> downstreamList2,ArrayList<Integer> levelDownStreamList2, ArrayList<Integer> processedList){
		...
		// List output protein in current reaction
		 for (int i3=0;i3<sRight1.length;i3++){
			  String name = main.PathwayViewer_2_9.getProteinName(sRight1[i3].toString());
			  if (name==null)
			  ...
			  else if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(name)!=null){
					  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(name);
					  ArrayList<String> components = main.PathwayViewer_2_9.proteinsInComplex[id];
	...
	public ArrayList<String> compareInputOutput(Object[] a, Object[] b){
		...
		for (int i=0; i<a.length;i++){
			...
			if (main.PathwayViewer_2_9.mapProteinRDFId.get(ref1)!=null){
				String proteinName1 =main.PathwayViewer_2_9. mapProteinRDFId.get(ref1);
				for (int j=0; j<b.length;j++){
					String ref2 = b[j].toString();
					String proteinName2 = main.PathwayViewer_2_9.mapProteinRDFId.get(ref2);
					if (proteinName2!=null && proteinName1.equals(proteinName2) 
							&& !main.PathwayViewer_2_9.mapSmallMoleculeRDFId.containsValue(proteinName1)){
							 results.add(proteinName1);
					}	
				}
			}
			else if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref1)!=null){
				int index1 = main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref1);
				String cName1 = main.PathwayViewer_2_9.complexList.get(index1).getDisplayName();
				for (int j=0; j<b.length;j++){
					String ref2 = b[j].toString();
					if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref2)==null)
						continue;
					int index2 = main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref2);
					String cName2 = main.PathwayViewer_2_9.complexList.get(index2).getDisplayName();
	...
	public ArrayList<String> compareInputOutput(Object[] a, Object[] b){
		...
		for (int i=0; i<a.length;i++){
			...
			for (int j=0; j<b.length;j++){
				...
				if (str1.equals(str2)){
					 String name = main.PathwayViewer_2_8.getProteinName(str1);
					 if (!main.PathwayViewer_2_8.isSmallMolecule(name)){
						 results.add(str1);
					 }	 
				}	
			}
	...
	public void drawProteinLeft(int p, float sat) {
		...
		for (int i=0;i<interElements.size();i++){
			...
			if (curLevel==level){
				String ref = interElements.get(i);
				String interProteinName = main.PathwayViewer_2_9.getProteinName(ref);
	...
	public void drawReactionLink(BiochemicalReaction rect, int i2, float xL, float xL2, float xRect, float xR, float xR2, float sat) {
		...
		float yReact = iY[i2].value;
		  for (int i3=0;i3<sLeft.length;i3++){
			  String name = main.PathwayViewer_2_9.getProteinName(sLeft[i3].toString());
			  if (name==null)
				  name = sLeft[i3].toString();

			  if (mapProteinRDFId_index.get(name)!=null){
				  float y5 = iP[mapProteinRDFId_index.get(name)].value-hProtein/4f;
				  if (check11.s && main.PathwayViewer_2_9.isSmallMolecule(name) && sat==200)
					  drawGradientLine(xL, y5, xRect, yReact, smallMoleculeColor, sat);
				  else if (check15.s && !main.PathwayViewer_2_9.isSmallMolecule(name) && sat==200){
			  ...
			  // Complex LEFT
			  else if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(sLeft[i3].toString())!=null){
				  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(sLeft[i3].toString());
		  ...
		  for (int i3=0;i3<sRight.length;i3++){
			  String name = main.PathwayViewer_2_9.getProteinName(sRight[i3].toString());
			  if (name==null)
			  ...
			  else if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(sRight[i3].toString())!=null){
				  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(sRight[i3].toString());
		...
		boolean result = false;
		  ...
		  if (processedComplexLeft.indexOf(id)<0 || sat==255){  // if not drawn yet
			  ...
			  if (check5.s && (sat==255 || (PopupCausality.s==4 && selectedComplexForCommonDownstream.contains(id)))){
				  parent.textAlign(PApplet.CENTER);
				  parent.textSize(12);
				  parent.text(main.PathwayViewer_2_9.complexList.get(id).getDisplayName(),xL2,yL2-5);
			  }
		  }
		 ...
		 ArrayList<String> components = main.PathwayViewer_2_9.proteinsInComplex[id];
		  ...
		  if (processedComplexRight.indexOf(id)<0 || sat==255){  // if not drawn yet
			  ...
			  if (sat==255 && check5.s){
				  parent.textAlign(PApplet.CENTER);
				  parent.textSize(12);
				  parent.text(main.PathwayViewer_2_9.complexList.get(id).getDisplayName(),xR2,yR2-5);
			  }
			  ...
			  // Draw connecting Complex for simulations
				...
				for (int i=0;i<interElements.size();i++){
					...
					if (curLevel==level){
						String ref = interElements.get(i);
						if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref)!=null){
							  int complexId = main.PathwayViewer_2_9.mapComplexRDFId_index.get(ref);
							  if(complexId==id){
									float x1 = xL2;
									float x2 = xR2;
									float max = 0;
		...
		for (int c=0;c<yComplexes.length;c++){
			...
			if (PApplet.dist(xL2,yComplexes[c].value, parent.mouseX, parent.mouseY)<=rComplexes[c]){
				...
				for (int r=0;r<rectList.size();r++) {
					...
					Object[] sLeft = rect.getLeft().toArray();
					for (int i3=0;i3<sLeft.length;i3++){
						  if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(sLeft[i3].toString())!=null){
							  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(sLeft[i3].toString());
		...
		for (int c=0;c<yComplexes.length;c++){
			...
			if (PApplet.dist(xR2,yComplexes[c].value, parent.mouseX, parent.mouseY)<=rComplexes[c]){
				...
				for (int r=0;r<rectList.size();r++) {
					...
					Object[] sRight = rect.getRight().toArray();
					for (int i3=0;i3<sRight.length;i3++){
						  if (main.PathwayViewer_2_9.mapComplexRDFId_index.get(sRight[i3].toString())!=null){
							  int id = main.PathwayViewer_2_9.mapComplexRDFId_index.get(sRight[i3].toString());
		...
		if (buttonStop.b){
			...
			if (PopupCausality.s==4){
				if (bProteinL>=0 || bComplexL>=0){
					...
					System.out.println("selectedReactionsForCommonDownstream="+selectedReactionsForCommonDownstream);
	...
	public void resetSelectionSimulation() {
		if (bRect>=0){
			...
			for (int i=0;i<bRectListL.size();i++){
				...
				simulationRectListLevelAll.add(0);
				System.out.println("bProteinL="+bProteinL);

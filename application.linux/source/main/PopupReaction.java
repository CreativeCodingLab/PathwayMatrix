...
public class PopupReaction{
	...
	public void setItems(){
		...
		s=-400;
		for (BiochemicalReaction current : main.PathwayViewer_1_7.reactionSet){
	...
	public void updateComplexList(){
		...
		for (int r=0;r<rectList.size();r++) {
			...
			for (int i3=0;i3<aLeft.length;i3++){
				  if (main.PathwayViewer_1_7.mapComplexRDFId_index.get(aLeft[i3].toString())!=null){
					  int id = main.PathwayViewer_1_7.mapComplexRDFId_index.get(aLeft[i3].toString());
					  if (id>maxID)
						  maxID =id;
			...
			for (int i3=0;i3<aRight.length;i3++){
				  if (main.PathwayViewer_1_7.mapComplexRDFId_index.get(aRight[i3].toString())!=null){
					  int id = main.PathwayViewer_1_7.mapComplexRDFId_index.get(aRight[i3].toString());
					  if (id>maxID)
						  maxID =id;
					  if(complexList.indexOf(id)<0)
						  complexList.add(id);
	...
	public int[][] computeScoreComplex(){
		...
		for (int c=0;c<complexList.size();c++){
			 ArrayList<String> components = main.PathwayViewer_1_7.proteinsInComplex[complexList.get(c)];
			 for (int k=0;k<components.size();k++){
	...
	public void drawReactions(float x_){
		...
		if (proteins==null) return;
			...
			else if (bRect>=0 || !textbox1.searchText.equals("") || bRectListL.size()>0 || bRectListR.size()>0){
				for (int p=0; p<proteins.length;p++){
				...
				if (PApplet.dist(xL2,yComplexesL[c].value, parent.mouseX, parent.mouseY)<=rComplexesL[c]){
					...
					for (int r=0;r<rectList.size();r++) {
						...
						for (int i3=0;i3<sLeft.length;i3++){
							  if (main.PathwayViewer_1_7.mapComplexRDFId_index.get(sLeft[i3].toString())!=null){
				...
				if (PApplet.dist(xR2,yComplexesR[c].value, parent.mouseX, parent.mouseY)<=rComplexesR[c]){
					...
					for (int r=0;r<rectList.size();r++) {
						...
						for (int i3=0;i3<sRight.length;i3++){
							  if (main.PathwayViewer_1_7.mapComplexRDFId_index.get(sRight[i3].toString())!=null){
								  int id = main.PathwayViewer_1_7.mapComplexRDFId_index.get(sRight[i3].toString());
			...
			if (PopupCausality.s==0 || PopupCausality.s==2){
				...
				for (int i=0;i<interElements.size();i++){
					...
					String name = main.PathwayViewer_1_7.getProteinName(ref);
					  if (name==null){
					  ...
					  else if (main.PathwayViewer_1_7.mapComplexRDFId_index.get(ref)!=null){
						  int id = main.PathwayViewer_1_7.mapComplexRDFId_index.get(ref);
						  name = main.PathwayViewer_1_7.proteinsInComplex[id].toString();
	...
	public void computeShortestPath(){
		...
		if (bProteinL>=0){
			...
			for (int i=0;i<proteinsDownStreamList.size();i++){
				...
				for (SmallMolecule currentMolecule : main.PathwayViewer_1_7.smallMoleculeSet){
	...
	public void listProteinDownStream(int r, int recursive, ArrayList<Integer> downstreamList,ArrayList<Integer> levelDownStreamList, ArrayList<Integer> downstreamList2,ArrayList<Integer> levelDownStreamList2, ArrayList<Integer> processedList){
		...
		// List output protein in current reaction
		 for (int i3=0;i3<sRight1.length;i3++){
			  String name = main.PathwayViewer_1_7.getProteinName(sRight1[i3].toString());
			  if (name==null)
			  ...
			  else if (main.PathwayViewer_1_7.mapComplexRDFId_index.get(name)!=null){
					  int id = main.PathwayViewer_1_7.mapComplexRDFId_index.get(name);
					  ArrayList<String> components = main.PathwayViewer_1_7.proteinsInComplex[id];
	...
	public ArrayList<String> compareInputOutput(Object[] a, Object[] b){
		...
		for (int i=0; i<a.length;i++){
			...
			for (int j=0; j<b.length;j++){
				...
				if (str1.equals(str2)){
					 String name = main.PathwayViewer_1_7.getProteinName(str1);
					 if (!main.PathwayViewer_1_7.isSmallMolecule(name)){
						 results.add(str1);
					 }	 
				}	
			}
	...
	public void drawProteinLeft(int p, float sat) {
		...
		if (main.PathwayViewer_1_7.isSmallMolecule(proteins[p])){
		...
		boolean isContainedComplexL =false; // checking if there is a complex;
		  for (int i3=0;i3<sLeft.length;i3++){
			  ...
			  if (mapProteinRDFId_index.get(name)!=null){
				  ...
				  if (check11.s && main.PathwayViewer_1_7.isSmallMolecule(name) && sat==200)
					  drawGradientLine(xL, y5, xRect, yReact, smallMoleculeColor, sat);
				  else if (check15.s && !main.PathwayViewer_1_7.isSmallMolecule(name) && sat==200){
		  ...
		  Object[] sRight = rect.getRight().toArray();
		  for (int i3=0;i3<sRight.length;i3++){
			  String name = main.PathwayViewer_1_7.getProteinName(sRight[i3].toString());
			  if (name==null)
			  ...
			  if (mapProteinRDFId_index.get(name)!=null){
			  ...
			  else if (main.PathwayViewer_1_7.mapComplexRDFId_index.get(sRight[i3].toString())!=null){
				  int id = main.PathwayViewer_1_7.mapComplexRDFId_index.get(sRight[i3].toString());
				  ArrayList<String> components = main.PathwayViewer_1_7.proteinsInComplex[id];
				  ...
				  if (processedComplexRight.indexOf(id)<0 || sat==255){  // if not drawn yet
					  ...
					  if (sat==255 && check5.s){
						  parent.textAlign(PApplet.CENTER);
						  parent.textSize(12);
						  parent.text(main.PathwayViewer_1_7.complexList.get(id).getDisplayName(),xR2,yR2-5);
					  }
				  }	  
			  }
	...
	public void mouseClicked2() {
		if (popupCausality.b>=0){
			...
			if (PopupCausality.s==1){
				...
				ArrayList<Integer> a = getDirectReactionsOfProteinLeft(bProteinL);
				for (int i=0;i<a.size();i++){
					int r = a.get(i);
		...
		if (xButton<mX && mX<xButton+w1 && 0<=mY && mY<=yBegin){
			...
			for (int i=0; i<rectHash.size(); i++){
				if (xRect-50<=mX && mX<=xRect+50 && iY[i].value-iH[i].value/2<=mY && mY<=iY[i].value+iH[i].value/2){
					if (textbox1.searchText.equals("") || (!textbox1.searchText.equals("") && sRectListByText.indexOf(i)>=0)) {
						bRect =i;
						if (bRectOld!=bRect){
							resetIntegrators();
						}

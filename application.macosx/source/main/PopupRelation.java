...
public class PopupRelation{
	...
	public PopupRelation(PApplet parent_){
		parent = parent_;
		items = new String [MainViewer.minerList.size()];
		for (int i = 0; i < MainViewer.minerList.size(); i++) {
			items[i] = MainViewer.minerList.get(i).toString();
		}	
		s = new boolean[MainViewer.minerList.size()];
	}
	...
	public void draw(float x_){
		...
		if (b>=0){
			...
			float max =-1;
			for (int j=0;j<main.MainViewer.pairs.length;j++){
				float sqrt = PApplet.sqrt(main.MainViewer.pairs[j].size());
				if (sqrt>max)
					max = sqrt;
			}
			for (int i=0;i<items.length;i++){
				...
				if (s[i]){
					...
					parent.fill(main.MainViewer.colorRelations[i]);
				}
				else if (i==b){
					parent.fill(main.MainViewer.colorRelations[i]);
				}
				...
				float r = PApplet.map(PApplet.sqrt(main.MainViewer.pairs[i].size()), 0, max, 0, 25);

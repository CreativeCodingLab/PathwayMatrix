...
public class PopupRelation{
	...
	public PopupRelation(PApplet parent_){
		parent = parent_;
		items = new String [PathwayViewer_1_9.minerList.size()];
		for (int i = 0; i < PathwayViewer_1_9.minerList.size(); i++) {
			items[i] = PathwayViewer_1_9.minerList.get(i).toString();
		}	
		s = new boolean[PathwayViewer_1_9.minerList.size()];
	}
	...
	public void draw(float x_){
		...
		if (b>=0){
			...
			float max =-1;
			for (int j=0;j<main.PathwayViewer_1_9.pairs.length;j++){
				float sqrt = PApplet.sqrt(main.PathwayViewer_1_9.pairs[j].size());
				if (sqrt>max)
					max = sqrt;
			}
			for (int i=0;i<items.length;i++){
				if (s[i]){
					...
					parent.fill(main.PathwayViewer_1_9.colorRelations[i]);
				}
				else if (i==b){
					parent.fill(main.PathwayViewer_1_9.colorRelations[i]);
				}
				...
				float r = PApplet.map(PApplet.sqrt(main.PathwayViewer_1_9.pairs[i].size()), 0, max, 0, 25);
